...
void key_func_work(struct work_struct *work)
{
	int i = 100;
	while(i--)
	{
		printk("key_func_work is %d\n",i);
	}
}
...
int gpio_probe(struct platform_device *pdev)
{
	...
	//中断下文初始化
	//tasklet_init(&key_tasklet,key_func_tasklet,100);//tasklet初始化
	INIT_WORK(&key_work,key_func_work);//workqueue初始化
